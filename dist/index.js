...
// lib/pcb/create-soldermask-opening-layers.ts
...
function getCopperPourShape(pour, transform) {
  if (pour.shape === "rect") {
    const [cx, cy] = applyToPoint2(transform, [pour.center.x, pour.center.y]);
  ...
  if (pour.shape === "polygon") {
    ...
    const pointsString = pour.points.map((p) => applyToPoint2(transform, [p.x, p.y])).map((p) => `${p[0]},${p[1]}`).join(" ");
    return {
      elementType: "polygon",
...
// lib/pcb/create-soldermask-opening-layers.ts
var COPPER_TYPES = /* @__PURE__ */ new Set([
  "pcb_trace",
  "pcb_copper_pour",
  "pcb_smtpad",
  "pcb_plated_hole",
...
function createSoldermaskOpeningLayers({
  circuitJson,
  ctx,
  create
}) {
  ...
  for (const layer of ["bottom", "top"]) {
    ...
    const openings = circuitJson.filter(
      (element) => element.type === "pcb_soldermask_opening" && element.layer === layer
    );
    ...
    const copper = circuitJson.flatMap(
      (element) => COPPER_TYPES.has(element.type) ? create(element, copperContext) : []
    );
    objects.push({
      name: "g",
      type: "element",
      ...
      children: [
        ...
        ...shapes.map((shape, index) => ({
          ...
          attributes: {
            ...
            "data-pcb-layer": layer,
            "data-pcb-soldermask-opening-id": openings[index].pcb_soldermask_opening_id
          }
        })),
        {
          name: "g",
...
function createOpeningShape(opening, ctx) {
  const { transform } = ctx;
  const base = { type: "element", value: "", children: [] };
  if (opening.shape === "polygon") {
    return {
      ...base,
      name: "polygon",
      attributes: {
        points: opening.points.map((point) => applyToPoint3(transform, [point.x, point.y]).join(",")).join(" ")
      }
    };
  }
  const [x, y] = applyToPoint3(transform, [opening.x, opening.y]);
  ...
  return {
    ...
    attributes: {
      ...
      width: String(width),
      height: String(height),
      ...opening.shape === "rotated_rect" ? { transform: `rotate(${-opening.ccw_rotation} ${x} ${y})` } : {}
    }
  };
}
...
function createSvgObjectsFromPcbTraceError(pcbTraceError, circuitJson, ctx) {
  ...
  if (!port1 || !port2) {
    ...
    if (pcbTraceError.center) {
      const screenCenter = applyToPoint4(transform, {
  ...
  const screenPort1 = applyToPoint4(transform, {
    x: port1.x,
    y: port1.y
  });
  const screenPort2 = applyToPoint4(transform, {
    x: port2.x,
    y: port2.y
  });
  const errorCenter = {
...
function createSvgObjectsForViaTraceError(pcbTraceError, via, ctx) {
  ...
  if (pcbTraceError.center && via) {
    ...
    const screenVia = applyToPoint4(transform, {
      x: via.x,
      y: via.y
    });
...
function createSvgObjectsFromPcbFootprintOverlapError(error, circuitJson, ctx) {
  ...
  if (filteredReferencedElements.length > 0) {
    ...
    const screenCenter = applyToPoint5(transform, { x: centerX, y: centerY });
    svgObjects.push({
      name: "rect",
      type: "element",
      attributes: {
    ...
    for (const element of filteredReferencedElements) {
      const screenPos = applyToPoint5(transform, { x: element.x, y: element.y });
      svgObjects.push({
        name: "rect",
        type: "element",
        attributes: {
...
function createSvgObjectsFromPcbCourtyardOverlapError(error, circuitJson, ctx) {
  ...
  const screenMid = applyToPoint6(transform, { x: midX, y: midY });
  svgObjects.push({
    name: "rect",
    type: "element",
    attributes: {
  ...
  for (const center of componentCenters) {
    const screenPos = applyToPoint6(transform, center);
    svgObjects.push({
      name: "rect",
      type: "element",
      attributes: {
  ...
  if (componentCenters.length === 2) {
    const mapped = componentCenters.map((c) => applyToPoint6(transform, c));
    const s1 = mapped[0];
    const s2 = mapped[1];
    svgObjects.push({
      name: "line",
...
function createSvgObjectsFromPcbComponentOutsideBoardError(error, circuitJson, ctx) {
  ...
  if (!bounds) return [];
  const topLeft = applyToPoint7(transform, { x: bounds.min_x, y: bounds.min_y });
  const bottomRight = applyToPoint7(transform, {
    x: bounds.max_x,
    y: bounds.max_y
  });
...
function createSvgObjectsFromPcbTraceClearanceError(error, circuitJson, ctx) {
  ...
  const screenErrorCenter = applyToPoint8(
    transform,
    worldErrorCenter
  );
  const screenObstacleCenter = obstacleCenter ? applyToPoint8(transform, obstacleCenter) : void 0;
  const screenTraceEndpoints = traceEndpoints ? {
    start: applyToPoint8(
      transform,
      traceEndpoints.start
    ),
    end: applyToPoint8(transform, traceEndpoints.end)
  } : void 0;
...
import { applyToPoint as applyToPoint10, compose as compose2, rotate as rotate2, translate as translate2 } from "transformation-matrix";
...
function createSvgObjectsFromPcbFabricationNoteRect(fabricationNoteRect, ctx) {
  ...
  const [topLeftX, topLeftY] = applyToPoint11(transform, [
    center.x - halfWidth,
    center.y + halfHeight
  ]);
  const [bottomRightX, bottomRightY] = applyToPoint11(transform, [
    center.x + halfWidth,
    center.y - halfHeight
  ]);
...
function createSvgObjectsFromPcbFabricationNoteDimension(dimension, ctx) {
  ...
  const [lineStartX, lineStartY] = applyToPoint12(transform, [
    fromBase.x,
    fromBase.y
  ]);
  const [lineEndX, lineEndY] = applyToPoint12(transform, [toBase.x, toBase.y]);
  ...
  const createExtensionLine = (anchor) => {
    ...
    const [startX, startY] = applyToPoint12(transform, [anchor.x, anchor.y]);
    const [endX, endY] = applyToPoint12(transform, [endPoint.x, endPoint.y]);
    return {
      name: "path",
      type: "element",
      value: "",
  ...
  const [screenFromX, screenFromY] = applyToPoint12(transform, [
    fromOffset.x,
    fromOffset.y
  ]);
  const [screenToX, screenToY] = applyToPoint12(transform, [
    toOffset.x,
    toOffset.y
  ]);
  ...
  const [textX, textY] = applyToPoint12(transform, [textPoint.x, textPoint.y]);
  const transformedFontSize = font_size * Math.abs(transform.a);
  const children = [
    ...extensionSegments,
    {
...
function createSvgObjectsFromPcbNoteDimension(dimension, ctx) {
  ...
  const [lineStartX, lineStartY] = applyToPoint13(transform, [
    fromBase.x,
    fromBase.y
  ]);
  const [lineEndX, lineEndY] = applyToPoint13(transform, [toBase.x, toBase.y]);
  ...
  const createExtensionLine = (anchor) => {
    ...
    const [startX, startY] = applyToPoint13(transform, [anchor.x, anchor.y]);
    const [endX, endY] = applyToPoint13(transform, [endPoint.x, endPoint.y]);
    return {
      name: "path",
      type: "element",
      value: "",
  ...
  const [screenFromX, screenFromY] = applyToPoint13(transform, [
    fromOffset.x,
    fromOffset.y
  ]);
  const [screenToX, screenToY] = applyToPoint13(transform, [
    toOffset.x,
    toOffset.y
  ]);
  ...
  const [textX, textY] = applyToPoint13(transform, [textPoint.x, textPoint.y]);
  const transformedFontSize = font_size * Math.abs(transform.a);
  const children = [
    ...extensionSegments,
    {
...
function createSvgObjectsFromPcbNoteText(note, ctx) {
  ...
  const [x, y] = applyToPoint14(transform, [anchor_position.x, anchor_position.y]);
  const transformedFontSize = font_size * Math.abs(transform.a);
  const { textAnchor, dominantBaseline } = getSvgTextAnchorAlignment(
    anchor_alignment
  );
...
function createSvgObjectsFromPcbNoteRect(noteRect, ctx) {
  ...
  const halfWidth = width / 2;
  const halfHeight = height / 2;
  const [topLeftX, topLeftY] = applyToPoint15(transform, [
    center.x - halfWidth,
    center.y + halfHeight
  ]);
  const [bottomRightX, bottomRightY] = applyToPoint15(transform, [
    center.x + halfWidth,
    center.y - halfHeight
  ]);
  const rectX = Math.min(topLeftX, bottomRightX);
...
function createSvgObjectsFromPcbNoteLine(noteLine, ctx) {
  ...
  const [startX, startY] = applyToPoint17(transform, [x1, y1]);
  const [endX, endY] = applyToPoint17(transform, [x2, y2]);
  const baseStrokeWidth = typeof stroke_width === "number" ? stroke_width : 0;
  const transformedStrokeWidth = baseStrokeWidth * Math.abs(transform.a);
  const attributes = {
    x1: startX.toString(),
...
function createSvgObjectsFromPcbPlatedHole(hole, ctx) {
  ...
  const [x, y] = applyToPoint18(transform, [hole.x, hole.y]);
  ...
  if (hole.shape === "circular_hole_with_rect_pad") {
    ...
    const [holeCx, holeCy] = applyToPoint18(transform, [
  ...
  if (hole.shape === "pill_hole_with_rect_pad") {
    ...
    const [holeCenterX, holeCenterY] = applyToPoint18(transform, [
  ...
  if (hole.shape === "hole_with_polygon_pad") {
    ...
    const padPoints = padOutline.map(
      (point) => applyToPoint18(transform, [holeX + point.x, holeY + point.y])
    );
...
function createSvgObjectsFromPcbSilkscreenText(pcbSilkscreenText, ctx) {
  ...
  if (is_knockout) {
    ...
    const knockoutTransform = matrixToString3(
      compose3(
        translate3(transformedX, transformedY),
  ...
  return [
    {
      ...
      attributes: {
        ...
        "text-anchor": textAnchor,
        "dominant-baseline": dominantBaseline,
        transform: matrixToString3(textTransform),
...
function createSvgObjectsFromPcbCopperText(pcbCopperText, ctx) {
  ...
  if (!text) return [];
  const [ax, ay] = applyToPoint22(transform, [
    anchor_position.x,
    anchor_position.y
  ]);
  ...
  if (is_knockout) {
    ...
    const knockoutTransform = matrixToString5(
      compose4(
        translate4(ax, ay),
        rotate4(-ccw_rotation * Math.PI / 180),
  ...
  const textTransform = matrixToString5(
    compose4(
      translate4(ax, ay),
      rotate4(-ccw_rotation * Math.PI / 180),
      ...applyMirror ? [scale2(-1, 1)] : [],
...
function createSvgObjectsFromPcbSilkscreenLine(pcbSilkscreenLine, ctx) {
  ...
  const [transformedX1, transformedY1] = applyToPoint24(transform, [x1, y1]);
  const [transformedX2, transformedY2] = applyToPoint24(transform, [x2, y2]);
  const transformedStrokeWidth = stroke_width * Math.abs(transform.a);
  const color = layer === "bottom" ? colorMap2.silkscreen.bottom : colorMap2.silkscreen.top;
  return [
    {
...
function createSvgObjectsFromPcbCourtyardCircle(pcbCourtyardCircle, ctx) {
  ...
  const [transformedX, transformedY] = applyToPoint30(transform, [
    center.x,
    center.y
  ]);
  const transformedRadius = radius * Math.abs(transform.a);
...
var package_default = {
  ...
  devDependencies: {
    ...
    "circuit-json": "github:anil08607/circuit-json#6cab65770e02ac343e249117f3b584ffbaa1a4d5",
    esbuild: "^0.20.2",
    ...
    if (circuitJsonElm.type === "pcb_panel") {
      ...
      if (circuitJsonElm.shape === "polygon") {
        ...
        updateBounds({
          ...
          width: circuitJsonElm.width,
          height: circuitJsonElm.height,
          ccwRotationDegrees: circuitJsonElm.shape === "rotated_rect" ? circuitJsonElm.ccw_rotation : 0
        });
      }
...
function convertCircuitJsonToPcbSvg(circuitJson, options) {
  ...
  const circuitJsonForBounds = circuitJson.filter((element) => {
    ...
    if (element.type === "pcb_soldermask_opening") {
      return Boolean(options?.showSolderMask) && (!layer || element.layer === layer);
    }
    return true;
  });
  const {
